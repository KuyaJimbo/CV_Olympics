...
def GetBaselines(landmarks):
    ...
    """
    global Baseline_FloorY, Baseline_Height, Baseline_KneeToFoot
    ...
    # Calculate knee to feet height difference
    left_knee = landmarks[mp_pose.PoseLandmark.LEFT_KNEE]
    right_knee = landmarks[mp_pose.PoseLandmark.RIGHT_KNEE]
    avg_knee_y = (left_knee.y + right_knee.y) / 2
    Baseline_KneeToFoot = avg_knee_y - Baseline_FloorY
    
    print("Baselines Recalculated!")

def CheckDetectJump(threshold=0.1):
    """
    Check if a jump has occurred by comparing current foot position to baseline floor
    """
    ...
    return False  # Placeholder - implement actual jump detection logic

def CheckHeadLowered(landmarks, threshold=0.5):
    """
...
def CheckKneesBent(landmarks, threshold=0.3):
    """
    ...
    right_knee = landmarks[mp_pose.PoseLandmark.RIGHT_KNEE]
    avg_knee_y = (left_knee.y + right_knee.y) / 2
    return avg_knee_y > Baseline_FloorY + (Baseline_KneeToFoot * threshold)
...
def UpdatePoseDict(landmarks):
    ...
    # Maintain a list size of 6
    for key in pose_dictionary:
        if len(pose_dictionary[key]) > 6:
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            for name, state in pose_indicators:
                ...
                cv2.putText(image, name, (10, y_offset), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                y_offset += 30

            # Display jump details if recently detected
            if last_jump_details and time.time() - last_jump_details['time'] < jump_display_duration:
                jump_y_offset = y_offset + 30
                cv2.putText(image, f"Jump Value: {last_jump_details['jump_value']}", 
                            (10, jump_y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                jump_y_offset += 30
                cv2.putText(image, f"Block Type: {last_jump_details['block_type']}", 
                            (10, jump_y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
