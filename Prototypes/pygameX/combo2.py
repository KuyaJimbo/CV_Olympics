...
class PoseTracker:
    ...
    def detect_jump(self, landmarks):
        """Determine jump power and detect a jump."""
        current_time = time.time()

        # Set baseline height on first detection
        ...
        # Calculate jump power (vertical displacement)
        jump_distance = self.baseline_height - nose_y

        # Jump detection based on significant vertical distance
        if jump_distance > self.jump_threshold:
            self.last_jump_time = current_time
            return True, jump_distance * 3  # Multiply by factor to control jump height
...
def main():
    ...
    while running:
        ...
        if landmarks:
            ...
            # Jump detection
            should_jump, new_jump_power = pose_tracker.detect_jump(landmarks)
            
            # Only jump if not already jumping and jump is detected
            if should_jump and not is_jumping:
                is_jumping = True
                jump_count = 10
        ...
        if is_jumping:
            if jump_count >= -10:
                ...
                player_y -= (jump_count ** 2) * 0.5 * neg * jump_power
                jump_count -= 1
            else:
                jump_count = 10
                is_jumping = False
                player_y = HEIGHT - 300  # Reset to ground level
